// to build:
// gcc -o LRV6 LRV6.c -lpigpio -pthread -lbluetooth -L"/usr/sbin" -lmosquitto -lm
...
{		
	char clientid[24];
	unsigned short command[3] = { 0, 0, 1 };
	int bytes_read, x, y, switchVal = 1, prevSwitchVal = 1;
	...
	tim.tv_sec = 0;
	tim.tv_nsec = 200 * 1000 * 1000; // 200 ms
		...
		{	
			...
			if((running == 2) && (BTenabled == 1))
			{
				...
				{	
					...
					if(switchVal == 0 && prevSwitchVal == 1)
					{
						// init switch timer
						gpioSetTimerFunc(6, 250, switchTick);
					}
					else if(switchVal == 1 && prevSwitchVal == 0)
					{
						// stop switch timer
						...
						if(joyTick <= 4)
						{
							...
							else if(learnStatus == 3 && repeatStatus == 0)
							{
								initRepeatMode();
							}
						}
...
void repeatTimer(void)
{
	...
	if(tock % 50 == 0 && tock > 0 && processing == 0)
	{
		processing = 1;
		stop_motors();
		sleep(3);
